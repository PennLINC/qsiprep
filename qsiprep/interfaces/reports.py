...
\t\t\tThis run used <code>--dmri-biascorrect auto</code>, which decides whether to run
...
class DiffusionSummary(SummaryInterface):
    ...
    def _generate_segment(self):
        ...
        biascorrect = ''
        biascorrect_warning = ''
        if isdefined(self.inputs.dmri_biascorrect):
            biascorrect = self.inputs.dmri_biascorrect
            if biascorrect == 'auto':
...
from sub-1_dir-PA_dwi.nii.gz.
    ...
    desc = [
        f'A total of {num_groups} distortion group{plural} {plural2} included in the '
        f'{image_source} data. '
    ]
    for distortion_group, image_list in selected_per_warp_group.items():
        group_desc = [f"Distortion group '{distortion_group}' was represented by "]
        for image_name, image_indices in image_list:
            ...
            group_desc += [
                f'image{plural} {formatted_indices} from {op.split(image_name)[-1]}',
                ', ',
            ]
        group_desc[-1] = '. '
        if len(image_list) > 1:
...
def get_cnr_values(cnr_image, brain_mask):
    ...
    for strategy in strategies:
        ...
        for cnr_name, cnr_value in zip(cnr_labels, cnr_values, strict=False):
