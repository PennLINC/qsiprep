...
def local_bvec_rotation(original_bvecs, warp_transforms, mask_image, runtime, output_fname):
    ...
    b0_image = get_vector_nii(
        np.stack([np.zeros_like(mask_data)] * 3, -1), mask_img.affine, mask_img.header
    )
    commands = []
    rotated_vec_files = []
    for vecnum, (original_bvec, warp_file) in enumerate(
        ...
        rotated_vector_fname, rotate_cmd = reorient_tensor_image(
            tensor_fname, warp_file, mask_img, num_prefix, out_fname
        )
