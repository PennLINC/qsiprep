...
class B0RPEFieldmap(SimpleInterface):
    ...
    **Inputs**
    ...
    def _run_interface(self, runtime):
        ...
        # Get metadata from all the input images, honoring the inheritance principle
        b0_fieldmap_metadata = [
            load_sidecar(image_path) for image_path in sorted(set(original_files))
        ]
...
def _delta_te(in_values, te1=None, te2=None):
    ...
    # For convenience if both are missing we should give one error about them
...
def topup_inputs_from_4d_file(
    ...
    **Parameters**
    ...
    for b0_index, bids_file in zip(b0_indices, b0_bids_origins, strict=False):
...
class ApplyJacobianWeights(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        # There are a few unique weight images. Find them
