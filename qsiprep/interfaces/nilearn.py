...
class MaskEPI(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        if self.inputs.closing:
            closed = sim.binary_closing(masknii.get_fdata().astype(np.uint8), sim.ball(1)).astype(
                np.uint8
            )
            masknii = masknii.__class__(closed, masknii.affine, masknii.header)

        if self.inputs.fill_holes:
            filled = binary_fill_holes(masknii.get_fdata().astype(np.uint8), sim.ball(6)).astype(
                np.uint8
            )
...
class EnhanceB0(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        out_bias_corrected = fname_presuffix(
            self.inputs.b0_file, suffix="_unbiased", newpath=runtime.cwd
        )
        ...
        out_enhanced = fname_presuffix(
            self.inputs.b0_file, suffix="_unbiasedsharpened", newpath=runtime.cwd
        )
...
def calculate_gradmax_b0_mask(b0_nii, show_plot=False, quantile_max=0.8, pad_size=10, cwd=None):
    ...
    **Returns**
    ...
    if pad_size:
        padded_nii = run_imagemath(
            b0_nii, "PadImage", [str(pad_size)], copy_input_header=False, cwd=cwd
        )
    else:
        padded_nii = b0_nii
    ...
    clipped = robust_scale(values, quantile_range=(0, quantile_max), with_centering=False)
    scaled = np.clip(
        power_transform(clipped, method="box-cox", standardize=False).squeeze(), 0, None
    )
    cutoff = otsu(scaled)
    ...
    maurer_abs = new_img_like(
        ...
        np.abs(
            run_imagemath(
                new_img_like(padded_nii, data), "MaurerDistance", [], cwd=cwd
            ).get_fdata()
        ),
    )
    ...
    if pad_size:
        processed_mask = processed_mask[pad_size:-pad_size, pad_size:-pad_size, pad_size:-pad_size]
        scaled_image = scaled_image[pad_size:-pad_size, pad_size:-pad_size, pad_size:-pad_size]
        grad_data = grad_data[pad_size:-pad_size, pad_size:-pad_size, pad_size:-pad_size]
    ...
    if show_plot:
        ...
        display = plot_epi(b0_nii, cmap="gray", vmax=vmax, display_mode="z", cut_coords=10)
...
def watershed_refined_b0_mask(
    b0_nii, show_plot=False, pad_size=10, quantile_max=0.8, ribbon_size=5, cwd=None
):
    ...
    **Returns**
    ...
    if pad_size:
        initial_mask_nii = run_imagemath(
            initial_mask_nii, "PadImage", [str(pad_size)], copy_input_header=False, cwd=cwd
        )
        initial_scaled_nii = run_imagemath(
            initial_scaled_nii, "PadImage", [str(pad_size)], copy_input_header=False, cwd=cwd
        )
    ...
    definitely_outer = (
        ndimage.binary_dilation(dilated_mask, structure=sim.cube(morph_size)) ^ dilated_mask
    )
    ...
    inner_weights = 1.0 - inner_weights / inner_weights.max()

    # Down-weight data as it gets far from the mask
    maurer = run_imagemath(
        new_img_like(initial_mask_nii, eroded_mask), "MaurerDistance", [], cwd=cwd
    ).get_fdata()
    ...
    morph_grad = (
        ndimage.morphological_gradient(scaled_image, footprint=sim.cube(3)) * smoothed_weights
    )
    ...
    if pad_size:
        ws_mask = ws_mask[pad_size:-pad_size, pad_size:-pad_size, pad_size:-pad_size]
        morph_grad = morph_grad[pad_size:-pad_size, pad_size:-pad_size, pad_size:-pad_size]
    ...
    if show_plot:
        display = plot_epi(b0_nii, cmap="gray", display_mode="z", cut_coords=10)
        display.add_contours(mask_img, linewidths=2)
        disp2 = plot_epi(
            grad_img,
            cmap="gray",
...
def select_markers_for_rw(image, inner_mask, empty_mask, outer_mask, sample_proportion=0.5):
    markers = np.zeros_like(image) - 1.0
