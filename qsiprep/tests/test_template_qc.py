...
def _structured(shape=(28, 28, 28), shift=0):
    ...
    body = (gx >= 6 + shift) & (gx < 22 + shift) & (gy >= 6) & (gy < 22) & (gz >= 6) & (gz < 22)
    vol[body] = 60.0
    # internal features -- the thing a correlation can actually latch onto
    vol[
        (gx >= 10 + shift) & (gx < 14 + shift) & (gy >= 10) & (gy < 18) & (gz >= 10) & (gz < 18)
    ] = 140.0
    vol[
        (gx >= 16 + shift) & (gx < 19 + shift) & (gy >= 12) & (gy < 16) & (gz >= 12) & (gz < 16)
    ] = 20.0
    return vol
...
def template_set(tmp_path):
    ...
    good = [
        _write(tmp_path / f'good{i}.nii.gz', base + rng.normal(0, 2, base.shape)) for i in range(3)
    ]
...
def test_outlier_is_flagged(template_set, tmp_path):
    import pandas as pd
...
def test_agreement_map_is_written_on_the_template_grid(template_set, tmp_path):
    import nibabel as nb

    from qsiprep.interfaces.template_qc import TemplateQC

    template, images = template_set
    res = TemplateQC(aligned_images=images, template=template).run(cwd=str(tmp_path))
