...
[@yeh2013deterministic]. """ %  DSI_STUDIO_VERSION
    ...
    if qsirecon_suffix:
        # Save the output in the outputs directory
        ds_tracking = pe.Node(ReconDerivativesDataSink(qsirecon_suffix=qsirecon_suffix),
                              name='ds_' + name,
                              run_without_submitting=True)
...
def init_dsi_studio_connectivity_wf(omp_nthreads, available_anatomical_data, name="dsi_studio_connectivity",
    ...
    if qsirecon_suffix:
        # Save the output in the outputs directory
        ds_connectivity = pe.Node(ReconDerivativesDataSink(qsirecon_suffix=qsirecon_suffix),
                                  name='ds_' + name,
                                  run_without_submitting=True)
...
def init_dsi_studio_export_wf(omp_nthreads, available_anatomical_data, name="dsi_studio_export",
    ...
    recon_scalars = pe.Node(
        DSIStudioReconScalars(workflow_name=qsirecon_suffix),
        name="recon_scalars",
        n_procs=1)
    fixhdr_nodes = {}
