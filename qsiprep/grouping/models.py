...
class EstimationMethod(enum.StrEnum):
    ...
    ANAT_CONTRAST = 'anat_reg'  # fieldmap-less: registration to an anatomical
...
class DistortionSignature:
    ...
    def opposes(self, other: DistortionSignature, ignore_shims: bool = False) -> bool:
        """True if ``other`` is reverse phase-encoded relative to this signature."""
        if self.pe_axis is None or other.pe_axis is None:
            return False
        return (
            self.pe_axis == other.pe_axis
            and self.pe_polarity == -other.pe_polarity
            and self.compatible_shim(other, ignore_shims=ignore_shims)
        )
...
class FileRecord:
    ...
    datatype: str  # 'dwi' | 'fmap'
    suffix: str  # dwi|epi|fieldmap|phasediff|phase1|phase2|magnitude*|sbref
    session: str | None
...
class DWIGrouping:
    ...
    def estimation_for(self, dwi_file: str) -> FieldmapEstimation | None:
        b0field_id = self.application.get(dwi_file)
        return self.estimations[b0field_id] if b0field_id else None
